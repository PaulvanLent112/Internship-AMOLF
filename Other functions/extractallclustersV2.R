extractallclusters<- function(gn, clustersize, mylinkmat, mydim, Nref, falsecluster, falsemembers, output){
  require(pracma)
  require(zipfR)
  computenoisesizes5<-source("functions/computenoisesizes5.R")
  computingclusterflag<-source("functions/computingclusterflag.R")
  #Make supportfolder
  ...
  dimpercolsmooth<- dimpercolsmooth$y #I think this is what we are interested in 
  ...
  #Now this part works
  trackclustersize= matrix(NaN,nrow=gn-1,ncol=1)
  icount=0
  for (i in 1:(gn-1)){
  ...
  maxpercolthreshold=fzero(function(x) gn* Rbeta(sin(x*pi)^2, (mydim-2)/2, 0.5)/2- dimpercolsmooth[mydim-3],
                           mylinkmat[nrow(mylinkmat),3])$x  
  
  
  maxpercolind<- which.min(abs(maxpercolthreshold-mylinkmat[,3]))
  clusterIDreal<- matrix(NaN, nrow=1,ncol=icount)
    ...
    if (sum(mergepoints>0)==2){ #Only if clusters merge this can be a percolation point for the two branches, since then they are real clusters 
      ...
      if (clusterflag<2){
        ...
        if (sizethreshlist[mydimlist-3]==0){
          ...
          sizethreshmean<- size_compute$sizethreshmean
        ...
        checkIDlist<- mergepoints[smind]  #do we actually need to subtract gn?
        print(checkIDlist)
        checkflag=0
        ...
        while (!isempty(checkIDlist)){
          ...
          temp<- trackclustersize[1:checkIDlist[1] %in% trackclustersize[checkIDlist[1]]] #this probably doesnt work
          ...
          if (sum(abovethresh1)>0){ #this throws an error, probably because of temp variable
          ...
          if (realclustpos<checkIDlist[1]){ #there are branchings above
            ...
            tempnew=temp * c(rep(0, realclustpos),rep(1, (checkIDlist[1]-realclustpos)))# not sure what is going on here
            if (sum(tempnew)==0){
              print('error tempnew')
            }
            ...
            if (!isempty(mymergingspos)){ #cluster branching in
              checkIDlisttemp<- Reshape(mymergingspos,length(mymergingpos),1) #?????????
  ...
  return<- list(trackclustersize, allclusters)
}