...
%-falscluster: threshold to avoid false positive clusters, multiple of the
%STD for the expected maximum cluster size
%-falsemembers: trade-off between false positive and false negative rate
%for the cluster membership, threshold for the fraction between the cluster
%size and the largest background cluster
%-output: 0-no output, >0-number of found clusters, >1-plot of
%cluster growth relative to reference
%
%
%based on extractallclusters12 (now approximating high dimensions)
...
function [trackclustersize,allclusters,sizethreshlog]...
    = extractallclusters13(gn,clustersize,mylinkmat,mydim,Nref,...
    falsecluster,falsemembers,output)

%Cut-off parameter for approximation
maxdim=50;
    ...
    if sum(mergepoints>gn)==2 %only if clusters merge this can be a percolation point
        ...
        if clusterflag<2 %either both are not part of any real cluster or
            ...
            if sizethreshlist(mydim-3)==0
                if mydim>maxdim
                    if sizethreshlist(maxdim-3)==0
                        [~,sizethreshmean,sizethreshstd]=...
                            computenoisesizes5(gn,maxdim,Nref);
                        sizethreshlog{maxdim-3,1}=sizethreshmean;
                        sizethreshlog{maxdim-3,2}=sizethreshstd;
                        sizethreshlist(maxdim-3)=1;
                    end
                    sizethreshmean = asin(sqrt(betaincinv(...
                        betainc((sin(sizethreshlog{maxdim-3,1}*pi)).^2,...
                        (maxdim-2)/2,1/2)/dimpercolsmooth(maxdim-3)*...
                        dimpercolsmooth(mydim-3),(mydim-2)/2,1/2)))/pi;
                    sizethreshstd = sizethreshlog{maxdim-3,2}*sqrt(maxdim/mydim);
...
%                 abovethresh1(temp)=(mymergings(temp)<...
%                         sizethresh(myclustersize(temp)-1));
...
%            if checkflag>0
%                 disp([mylinkmat(i,3), sum(~isnan(clusterIDreal))])%\\\\\\\\
%                 disp(find(~isnan(clusterIDreal)))
%                 disp(clusterIDreal(~isnan(clusterIDreal)))
% %                 pause
%             end
